...
$redirectUrl = '../../views/backend/members/edit.php';
if (!empty($numMemb)) {
    $redirectUrl .= '?numMemb=' . urlencode($numMemb);
}

$recaptcha = verifyRecaptcha($_POST['g-recaptcha-response'] ?? '', 'update');
if (!$recaptcha['valid']) {
    $errors[] = $recaptcha['message'] ?: 'Échec de la vérification reCAPTCHA.';
}
...
if (isset($numMemb, $prenomMemb, $nomMemb, $eMailMemb, $numStat)) {
